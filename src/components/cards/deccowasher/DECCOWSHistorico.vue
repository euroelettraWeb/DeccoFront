...
const cargadoEstadoBomba = ref(false);
...
async function select(columna) {
  ...
  if (columna == "decco") {
    const lotDesinf = await obtenerDatosVariableGeneral(
      "todo",
      "valores",
      "individual",
      "sinfiltro",
    ...
    const lotJabon = await obtenerDatosVariableGeneral(
      "todo",
      "valores",
      "individual",
      "sinfiltro",
    ...
    lotes.value = [
      ...lotDesinf[0].registros.map((r) => {
        return {
          id: `LD_${r.loteDeccoDesinfectante}`,
          lote: `Lote Desinfectante: ${r.loteDeccoDesinfectante}`,
        };
      }),
      ...lotJabon[0].registros.map((r) => {
        return {
          id: `LJ_${r.loteDeccoJabon}`,
          lote: `Lote Jabón: ${r.loteDeccoJabon}`,
        };
      }),
    ];
  }
...
async function historico(date1, date2) {
  ...
  cargadoEstadoBomba.value = false;
  ...
  let lotesDeccoDesinfectante = await obtenerDatosVariableGeneral(
    "historico",
    "registros",
    "multiple",
    "formatoLotesDesinf",
  ...
  let lotesDeccoJabon = await obtenerDatosVariableGeneral(
    "historico",
    "registros",
    "multiple",
    "formatoLotesJabon",
  ...
  seriesLotes.value = [
    {
      name: "Lotes DeccoWasher",
      data: [...lotesCliente, ...lotesDeccoDesinfectante, ...lotesDeccoJabon],
    },
  ];
  ...
  seriesBombas.value = bombas;
  cargadoEstadoBomba.value = true;
  ...
  if (deccodos.value) {
    ...
    totalesConsumo.value = totalesConsumo.value.filter(
      (item) => item.name !== "Litros Agua" && item.name !== "Litros Agua(L/T)"
    );
